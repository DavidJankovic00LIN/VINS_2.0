  ...
  return (
      ...
      <nav className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
       ...
       <div className="hidden md:block relative">
            ...
            className="h-9 px-4 rounded-full border border-orange-500/40 bg-orange-600 text-white text-sm font-medium hover:bg-orange-700 hover:border-orange-500/60 hover:scale-105 transition-all flex items-center gap-2"
          >
           {lang}
           ...
           <div className="grid grid-cols-2 gap-2">
             {languages.map((lng) => (
               <button
                 key={lng.code}
                 onClick={() => {
                   selectLanguage(lng.code as 'ENG' | 'SRB' | 'TUR');
                   setIsMobileMenuOpen(false);
                 }}
                 className={`text-left px-3 py-2 text-sm rounded-lg transition-colors ${
                   lang === lng.code 
                     ? 'bg-orange-600 text-white font-medium' 
                     : 'bg-orange-500/10 text-white hover:bg-orange-500/20'
                 }`}
               >
                 {lng.code} - {lng.name}
               </button>
